...
export function findById(req: Request, res: Response) {
    ...
    Wine.findById(id).then((wine: Wine) => {
        if (wine)
            res.send(wine.toJSON());
        else
...
export function findAll(req: Request, res: Response) {
    Wine.all().then((wines: Wine[]) => {
        const jsonWines = wines.map(w => w.toJSON());
        res.send(jsonWines);
    });
};

export function addWine(req: Request, res: Response) {
    const wineData = req.body;
    console.log('Adding wine: ' + JSON.stringify(wineData));
...
export function updateWine(req: Request, res: Response) {
    const id = req.params.id;
    const wineData = req.body;
    console.log('Updating wine: ' + id, wineData);

    Wine.update(
        wineData, {
            where: { _id: id },
            returning: true
        }).then((ret: [number, Wine[]]) => {
            if (ret[0] !== 1) {
                res.send({'error':'An error has occurred while updating'});
            } else {
                res.send(ret[1][0].toJSON());
            }
        });
}

export function deleteWine(req: Request, res: Response) {
    const id = req.params.id;
    ...
    Wine.findById(id).then((wine: Wine) => {
        if (wine) {
            ...
            console.log(`Wine did not exist`);
            res.send(req.body);
        }
    });
}
